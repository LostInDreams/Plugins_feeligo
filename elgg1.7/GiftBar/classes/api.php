...
class Feeligo_Elgg17_Api extends FeeligoApi
{
  ...
  protected function __construct() {
    parent::__construct();
    // store the viewer
    $this->_adapter_viewer = new Feeligo_Model_Adapter_User (get_loggedin_user(),false);
    
    // store the subject
    $this->_adapter_subject = null;
    if (get_user(page_owner_entity()-> guid) !== false) {
      $u_subj =new Feeligo_Model_Adapter_User (page_owner_entity(),true) ;
  ...
  public function has_viewer(){
    return true;
    return get_logged_in_user()->guid!=false;
  }
  public function has_subject(){
    return false;
    return page_owner_entity()->guid!=false;
  }
